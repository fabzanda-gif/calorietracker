...
def test_outcome_aware_learning_is_exposed():
    response = client.get(
    ...
    assert prefs["mode_learning_source"] == "outcome"
    assert prefs["preferred_lens"] == "taste"
    assert prefs["lens_learning_source"] == "outcome"
    assert prefs["preferred_source"] == "delivery"
    assert prefs["source_learning_source"] == "outcome"
...
def test_outcome_aware_source_still_boosts_ranked_candidate():
    response = client.get(
        "/days/2026-09-01/meals/dinner/options",
        params={"mode": "order"},
    )
